...
def get_batches(image, label, queue_examples, batch_size, shuffle):
     if shuffle:
         image_batch, label_batch = tf.train.shuffle_batch([image, label],batch_size=batch_size,num_threads=1,
         capacity=queue_examples + 3 * batch_size,min_after_dequeue=queue_examples)
    
     else:
         image_batch, label_batch = tf.train.batch([image, label],batch_size=batch_size,num_threads=1,
         capacity=queue_examples + 3 * batch_size)

     return image_batch, label_batch

def get_files(image_filenames, label_filenames, batch_size):
     global image_h, image_w, image_c
     ...
     fraction_of_examples_in_queue = 0.4
     queue_examples = int(num_train_examples*fraction_of_examples_in_queue)
     print ('Filling queue with %d CamVid images before starting to train, will take a few minutes.' %queue_examples)

     return get_batches(reshaped_image, label, queue_examples, batch_size, shuffle=True)
